		...
		init: function(view, width, height){
			switch(view.update){
				...
				case 'kiss':
					...
					view.body.count = 0
					view.ontick = function(){
						if(stage.checkpoint.kiss){
							this.kill()
						}
						this.t = this.body.count
						if(this.t > 3){
							stage.checkpoint.kiss = true
						}
					}
						...
						if(this.body.grab > 0){
							...
							if(this.grabbed){
								if(this.tossed){
									...
									ball.body.SetLinearVelocity(vec(11,0))
									this.tossed = true
								}
