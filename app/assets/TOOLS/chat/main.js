//(function(){
	...
	let classes = {
		...
		10.3: ['Aaron', 'Allen', 'Arie', 'Bob Wang', 'Dale', 'George Zhang', 'James Hu', 'Lori', 'Rebecca Pan', 'Susan', 'Tony Wang', 'Zack'],
	...
	let students = {
		...
		add_line: function(line){
			...
			if(stud.lines){
				...
				if(!this.active_classes[stud.cls]){
					this.active_classes[stud.cls] = true 
					console.log('%c ' + stud.name + ' activates ' + stud.cls  , 'color:red; background-color: black;')
				}
			}else{
				console.log(stud)
			}
		...
		match_student: function(line){
			if(!line) return 
			//console.log(line)
			function remove_dupes(matches, func, label){
				...
				if(matches.length < count){
					console.log("Removed dupe for", matches[0].name, "by way of", label)
				}
