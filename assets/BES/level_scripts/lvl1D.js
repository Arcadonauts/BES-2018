console.log('Level 1D Script loaded');
// Jumper
...
Player.prototype.update = function(){
	...
	*/
	...
	this.x += this.vx - game.v
	this.y += this.vy

	if(this.x + this.width > game.tw * (game.cols - 1)){
		this.vx = -.5*Math.abs(this.vx)
		this.x = - this.width + game.tw * (game.cols - 1)
	}
...
game = {start: function(){
		...
		gaps: function(diff){
			var col = []
			var solid = Math.random() + .1 > diff
			var h = Math.floor(Math.random()*3)
			for(var j = 0; j < this.rows; j++){
				if(solid && j >= this.rows - 2 - h){
					col[j] = new Tile()
				}else{
					col[j] = undefined
				}
